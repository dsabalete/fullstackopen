  ...
  const handleSubmit = (e) => {
    ...
    if (newName.length) {
      if (persons.map((person) => person.name).includes(newName)) {
        ...
        if (resp) {
          const prevPerson = persons.filter((p) => p.name === newName)[0]
          ...
          update(personObj.id, personObj)
            .then((response) => {
              ...
              notify(`${updatedObj.name}'s number has been updated`)
            })
            .catch((error) => {
              notifyError(
  ...
  const handleDeletePerson = (person) => {
    ...
    if (resp) {
      remove(person.id)
        ...
        .catch((error) => {
          notifyError(`Something went wrong while removing person in phonebook`)
        })
    }
  }
